...
if [[ "$ID" == "centos" && "$VERSION_ID" == "7" ]]; then
  ...
  #set proxy: /vagrant/scripts/docker-setup-proxy.sh
  #set overlayFS: /vagrant/scripts/docker-setup-storage_driver_overlayfs.sh

  echo "INFO: [dcos-mesos-install_prereqs.sh] Setting pre-reqs for DC/OS"
  sudo yum install -y tar xz unzip curl ipset net-tools
